...
> - **① 계승(inherit) = 디폴트** — 필요한 값이 루트 디자인 토큰(**값 SSOT = `index.html`의 `:root` 2블록**)에 이미 있으면 그 토큰을 `var()`로 참조. raw hex·px·blur·rgba·색 창작 절대 금지.
> - **② 갱신(update)** — `:root`에 없거나 기존 값을 바꿔야 하면 루트 토큰을 갱신. ⚠️ 갱신은 반드시 운영자에게 **먼저 묻고 승인**받은 뒤에만.
>
...
- **🎨 브랜드 컬러 확립본(팔레트 폐쇄형) = 기틀 §0 (260704 정립)** — 코어 인디고 `--accent`(=`--c1`) · 서브코어 살몬 `--peach-text`(=`--c2`, 피치 틴트 사다리) · 카테고리 축 `--c1~--c6`(유일 시리즈 팔레트, 동값 별칭 공존=정상) · 역할색 각 딱 하나(초록 `--green`·카카오 `#F5B400`·**레드는 미확립=운영자 결정 대기**) · 중립 사다리. 시각본 `docs/브랜드컬러_확립본.html`.
...
- **정립 3점**: ① **CLAUDE.md 최상단 제1 절대명령**(계승 `var()` or 운영자 승인갱신, 둘뿐 — 세 번째 없음 + 판정 절차 3단 + 팔레트 폐쇄) ② **기틀 §0 브랜드 컬러 확립본** — 코어 인디고 `--accent`(=`--c1`) / 서브코어 살몬 `--peach-text`(=`--c2`) / 카테고리 축 `--c1~--c6`=유일 시리즈 팔레트(동값 별칭 `--c3`=`--green`·`--c4`=`--insta`·`--c6`=`--kakao`승 → 정상 공존) / 역할색 각 딱 하나(**레드 미확립** = `#E24B4A` 40회 vs `#e5484d` 30회, `--danger` 없음 → 운영자 결정 대기 플래그) / 중립 사다리 ③ **시각본 `docs/브랜드컬러_확립본.html`**(라이트, 스냅샷 260704). `design_digest.py` 계약에 0)제1명령 줄 추가.
...
- **✅ v11 — 행정구역 색칠 + 도시 숫자원(운영자 260704 "도시는 원안에 숫자, 행정구역 20% 색칠: 여수 살몬(강조색2)·순천 파스텔그린·광양 파스텔핑크")**: ① 도시 집계 배지(핀·필)를 **원 안에 숫자 + 도시명 라벨**로 교체(`_pmapCityCircleHtml`, 색 토큰 여수 `--peach-text`·순천 `--c3`·광양 `--c4`, 크기=거점수). ② 오버레이에 **도시 행정구역(행정동 union) 20% 채우기** 추가 — baked PNG라 색 자유(살몬 240,196,184 / 파스텔그린 150,205,160 / 파스텔핑크 242,166,196), 도로 20%·동경계 50% 위에 깔림. `bake_overlay.py`에 city fill 레이어(mask→alpha 51). 게이트 1824 유지(원 색은 기존 토큰). ③ **좌측 메뉴 블릿 단순화**(사용자 "블릿이 너무 다채로워서 헷갈림") — 트리 행 카테고리 색점(스팟·매체·DID그룹) + 상단 범례 제거, 화살표+이름+비율칩만. 색 범례는 지도 필터칩·마커가 담당(색 정보 지도 한 곳으로).
