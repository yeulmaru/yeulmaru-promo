...
- **✅ v11 — 행정구역 색칠 + 도시 숫자원(운영자 260704 "도시는 원안에 숫자, 행정구역 20% 색칠: 여수 살몬(강조색2)·순천 파스텔그린·광양 파스텔핑크")**: ① 도시 집계 배지(핀·필)를 **원 안에 숫자 + 도시명 라벨**로 교체(`_pmapCityCircleHtml`, 색 토큰 여수 `--peach-text`·순천 `--c3`·광양 `--c4`, 크기=거점수). ② 오버레이에 **도시 행정구역(행정동 union) 20% 채우기** 추가 — baked PNG라 색 자유(살몬 240,196,184 / 파스텔그린 150,205,160 / 파스텔핑크 242,166,196), 도로 20%·동경계 50% 위에 깔림. `bake_overlay.py`에 city fill 레이어(mask→alpha 51). 게이트 1824 유지(원 색은 기존 토큰).
