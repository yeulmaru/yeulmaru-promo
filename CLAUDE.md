...
- **DB 통합 Phase 2 준비 완료 (쓰기만 대기)**: `_salesBuild` 공연ID 우선 조인(컬럼 없으면 기존 이름 조인 — 전후 호환). `docs/260610_db_migrate.mjs`(dry-run 기본 / `DB_PIN=핀 --write`로 시트 3개 교체: 일일입력+공연ID, 운영대장(정리)+공연ID, 운영_공연색인 신설). 매칭: 일일입력 77.6%(2,097/2,702행, 공연 26/36) · 운영대장 100% · 색인 1,306건. 미매칭 10공연=운영대장 부재(빈칸 유지, 진단의 unknown 10공연과 일치). **서브admin PIN 받으면 실행.**
...
- **다음 단계**: ① DB 통합 실행(PIN 대기) ② 모바일 Phase B(현황 보드/테이블 모바일, 우클릭→길게 누르기 대체) ③ 규정 PDF 챗봇. 글래스모피즘 리스킨은 시안 후 **폐기 결정**(현 UI 유지).
