...
- **✅ 모바일 Phase B 완료**: 현황 보드 3종 풀스크린, 길게 누르기(550ms)→컨텍스트 메뉴 합성(iOS 폴백·안드로이드 중복 방지 가드), 모달 글씨 상향. **✅ 규정 PDF 챗봇 코드 완료**: PDF 185p→조항 535행/19규정 파싱(docs/260610_rules_rows.json), Worker /api/chatbot/rules GET/POST(시트 '규정' 자동생성), 챗봇 FAQ 미스→규정 검색 폴백. **⚠️ Worker 재배포 후 docs/260610_rules_ingest.mjs 실행 필요** (인제스트 전까진 규정 검색이 빈 결과→미응답 폴백, 무해). 글래스모피즘 리스킨은 시안 후 **폐기 결정**(현 UI 유지).
