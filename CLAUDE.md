...
- **🎨 브랜드 컬러 확립본(팔레트 폐쇄형) = 기틀 §0 (260704 정립)** — 코어 인디고 `--accent`(=`--c1`) · 서브코어 살몬 `--peach-text`(=`--c2`, 피치 틴트 사다리) · 카테고리 축 `--c1~--c6`(임의 시리즈 전용 — 의미색 맵 `_BIZ_*_COLORS`는 별도 동결 §6-6, 동값 별칭 공존=정상·신규 별칭은 승인 필수) · 역할색 각 딱 하나(초록 `--green`·카카오 `#F5B400`·**레드는 미확립=운영자 결정 대기**) · 중립 사다리. 시각본 `docs/브랜드컬러_확립본.html`.
